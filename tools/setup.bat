...
if "%APPVEYOR_BUILD_WORKER_IMAGE%"=="Visual Studio 2013" ( 
    set _BUILD_TOOL_VERSION=120 && set image=VS2013 
)
if "%APPVEYOR_BUILD_WORKER_IMAGE%"=="Visual Studio 2015" (
    set _BUILD_TOOL_VERSION=140 && set image=VS2015 
)
if "%APPVEYOR_BUILD_WORKER_IMAGE%"=="Visual Studio 2017" ( 
    set _BUILD_TOOL_VERSION=141 && set image=VS2017 && C:\Program^ Files^ ^(x86^)\Microsoft^ Visual^ Studio\2017\Community\Common7\Tools\VsDevCmd.bat 
)
if "%APPVEYOR_BUILD_WORKER_IMAGE%"=="Visual Studio 2019'" ( 
    set _BUILD_TOOL_VERSION=142 && set image=VS2017 && C:\Program^ Files^ ^(x86^)\Microsoft^ Visual^ Studio\2017\Community\Common7\Tools\VsDevCmd.bat 
)
