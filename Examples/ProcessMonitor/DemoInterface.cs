...
namespace ProcessMonitor
{
    ...
    public class DemoInterface : MarshalByRefObject
    {
        ...
        {
            MessageBox.Show(e.ToString(), "A client process (" + InClientPID + ") has reported an error...",
        ...
        public void OnCreateFile(Int32 InClientPID, String[] InFileNames)
        {
            if (Form1.IsMonitoring)
            {
                lock(Form1.MonitorQueue)
                {
                    ...
                    {
                        Form1.MonitorQueue.Enqueue(new MonitorEntry(InClientPID, "[FILE]: \"" + InFileNames[i] + "\""));
                    }
                }
            }
        }
