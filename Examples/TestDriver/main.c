...
NTSTATUS TestDriverDispatchCreate(IN PDEVICE_OBJECT InDeviceObject, IN PIRP InIrp);

NTSTATUS TestDriverDispatchClose(IN PDEVICE_OBJECT InDeviceObject, IN PIRP InIrp);

NTSTATUS TestDriverDispatchDeviceControl(IN PDEVICE_OBJECT InDeviceObject, IN PIRP InIrp);
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT InDriverObject, IN PUNICODE_STRING InRegistryPath)
{
    ...
    /*
            Create device...
    */
    if (!NT_SUCCESS(Status = IoCreateDevice(InDriverObject,
