...
#define FORCE(expr) \
    { \
        if (!NT_SUCCESS(NtStatus = (expr))) goto ERROR_ABORT; \
    }
...
#pragma warning(disable : 4152)
    FORCE(Interface.LhInstallHook(MmGetSystemRoutineAddress(&SymbolName), KeCancelTimer_Hook, (PVOID)0x12345678, &hHook));
